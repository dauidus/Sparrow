...
class pwvintage_recent_comments extends WP_Widget {
    /** constructor */
    function pwvintage_recent_comments() {
        parent::WP_Widget(false, $name = 'PWvintage Comments');	
    }
...
function recent_comments($no_comments = 4, $comment_len = 50) {
	...
	if ($comments) {
		...
		foreach ($comments as $comment) {
			...
			?>	
				<div class="sidebar-item-box">
					...
					<a class="sidebar-item-title" href="<?php echo get_permalink( $comment->comment_post_ID ) . '#comment-' . $comment->comment_ID; ?>"><?php echo complete_get_author($comment); ?></a>
					on <a class="sidebar-item-title" href="<?php echo get_permalink( $comment->comment_post_ID ) . '#comment-' . $comment->comment_ID; ?>"><?php echo get_the_title( $comment->comment_post_ID );?></a>
		...
		echo '<li>'.__('No comments','pwvintage').'';
	}
...
add_action('widgets_init', create_function('', 'return register_widget("pwvintage_recent_comments");'));
?>